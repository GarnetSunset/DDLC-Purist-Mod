...
#Just add Monika art now!
    ...
    add "menu_bg"
        #add "menu_art_y"
        #add "menu_art_n"
    if persistent.yuriRouteStarted:
        add "menu_art_y"
    if persistent.natsukiRouteStarted:
        add "menu_art_n"
    if persistent.sayoriRouteStarted:
        add "menu_art_s"
    frame:
        pass
...
#        if persistent.playthrough != 4:
    if persistent.monikaRouteStarted:
        add "menu_art_m"
    add "menu_fade"
...
screen inputCode(prompt, someText = ""):
    window:
        style "nvl_window"
        text prompt xalign 0.5 yalign 0.4 
        input id "inputCode" style "input" default someText xalign 0.5 yalign 0.5
